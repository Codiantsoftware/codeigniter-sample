...
class Ion_auth
{
    ...
    public function register($identity, $password, $email, $additional_data = [], $group_ids = [])
    {
        ...
        if ($group_ids[0] == 2) {
            $wallet_balance = (isset($settings['welcome_wallet_balance_on']) && $settings['welcome_wallet_balance_on'] && isset($settings['wallet_balance_amount']) && $settings['wallet_balance_amount'] != '') ? $settings['wallet_balance_amount'] : '0';
            $additional_data['balance'] = $wallet_balance;
        } else {
            $wallet_balance = 0;
        }
    ...
    {
        return $this->ion_auth_model->in_group('delivery_boy', $id);
    }
